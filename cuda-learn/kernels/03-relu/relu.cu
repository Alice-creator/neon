...
__global__ void relu(const float* params, float* result, int n) {
    int index = blockDim.x * blockIdx.x + threadIdx.x;
    if (index >= n) return;
    result[index] = fmaxf(0.0f, params[index]);
}
...
int main() {
    ...
    relu<<<blocks, threads_per_block>>>(device_params, device_result, n);
    // ↑ <<< >>> không phải << >>                              ↑ thêm n
